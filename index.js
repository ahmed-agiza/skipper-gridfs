...
module.exports = function SkipperGridFS(globalOptions) {
	...
	const fileTransformer = options.transformer;
	...
	adapter.ls = (dirpath, cb) => {
		...
		client(options.uri, options.mongoOptions, (err, client) => {
			...
			const stream = bucket(client.db(), options.bucketOptions)
				.find({ 'metadata.dirname': dirpath })
				.transformStream();
	...
	adapter.read = (fd, cb) => {
		...
		client(options.uri, options.mongoOptions, (err, client) => {
			...
			).openDownloadStream(fd);
			downloadStream.once('end', () => {
				__transform__.emit('done', client);
			});
			downloadStream.once('error', (error, client) => {
				__transform__.emit('error', error, client);
			});
		...
		receiver__.write = (__newFile, encoding, done) => {
			client(options.uri, options.mongoOptions, (error, client) => {
				...
				const outs__ = bucket(
					client.db(),
					options.bucketOptions
				).openUploadStreamWithId(fd, filename, {
					metadata: {
						filename: filename,
						fd: fd,
				...
				if (fileTransformer) {
					__newFile.pipe(fileTransformer).pipe(outs__);
				} else {
